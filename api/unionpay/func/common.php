...
function decrypt_post_data($post_data) {
	global $PRIVATE_KEY_PATH;
	$priv_key = file_get_contents($PRIVATE_KEY_PATH);
	$res = openssl_get_privatekey($priv_key);
	openssl_private_decrypt(base64_decode($post_data), $decrypted_post_data, $res);
	return $decrypted_post_data;
}

function payMoney(){
	return $_COOKIE['money'];
}


function paySuccess($oID){
	global $DB_HOST,$DB_USER_NAME,$DB_PASSWORD,$DB_DATABASE,$DB_PORT;
	$con  = mysql_connect($DB_HOST,$DB_USER_NAME,$DB_PASSWORD,$DB_DATABASE,$DB_PORT) or die("Error " . mysqli_error($con)); 

	$sql  = "update bdm0330336_db.order set checkOrder = 1 where id = ".mysql_real_escape_string($oID); 

	$fp = fopen('../../../myfolder/files/callback_pay_success_sql.txt','w+');
	fwrite($fp, $sql);
	fclose($fp);
